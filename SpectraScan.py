...
COMMON_SUBDOMAINS = [
    ...
    'api', 'cdn', 'smtp', 'pop', 'imap', 'ssh', 'vpn',
    'cloud', 'backup', 'staging', 'demo', 'old', 'new'
]
HTTP_PATHS = [
    '/', '/admin', '/login', '/wp-admin', '/phpmyadmin',
    '/api', '/api-docs', '/swagger', '/console', '/manager',
    '/backup', '/wp-login.php', '/admin/login', '/administrator',
    '/.env', '/config', '/config.php', '/settings', '/dashboard',
...
def grab_banner(ip: str, port: int, timeout: float = 3.0) -> str:
    ...
    try:
        ...
        probes = {22: b"SSH-2.0-OpenSSH_8.0\r\n", 80: b"HEAD / HTTP/1.0\r\n\r\n", 443: b"HEAD / HTTP/1.0\r\n\r\n"}
        if port in probes:
...
def check_vulnerabilities(port: int, service: str, banner: str = "") -> List[Dict]:
    ...
    if port in VULNERABILITIES:
        for vuln in VULNERABILITIES[port]:
            severity = "HIGH" if any(k in vuln.lower() for k in ["unencrypted", "exposed", "vuln"]) else "MEDIUM"
            vulns.append({"port": port, "service": service, "vulnerability": vuln, "severity": severity, "cve": "N/A"})
    return vulns
...
class SYNScan:
    ...
    def create_packet(self, src_ip: str, dst_ip: str, dst_port: int) -> bytes:
        ...
        tcp_header = struct.pack('!HHLLBBHHH', random.randint(1024, 65535), dst_port,
                                random.randint(0, 4294967295), 0, 0x50, 0x02, 65535, 0, 0)
...
class UDPScan:
    ...
    def __init__(self):
        self.common_udp_ports = {
            53: ("DNS", "DNS Query"), 67: ("DHCP", "DHCP Server"), 68: ("DHCP", "DHCP Client"),
            69: ("TFTP", "Trivial FTP"), 123: ("NTP", "Network Time Protocol"),
            161: ("SNMP", "Simple Network Management"), 162: ("SNMPTRAP", "SNMP Trap"),
            389: ("LDAP", "Lightweight Directory Access"), 500: ("ISAKMP", "Internet Security"),
            514: ("Syslog", "System Logging"), 520: ("RIP", "Routing Information"),
            1194: ("OpenVPN", "VPN"), 1701: ("L2TP", "Layer 2 Tunneling"),
    ...
    def scan(self, target_ip: str, port: int, timeout: float = 3.0) -> Dict:
        ...
        try:
            ...
            probes = {53: b'\x00\x00\x01\x00\x00\x01\x00\x00\x00\x00\x00\x00\x06google\x03com\x00\x00\x01\x00\x01',
...
class OSFingerprint:
    ...
    def get_ttl(ip: str) -> Optional[int]:
        try:
            cmd = ['ping', '-n', '1', '-w', '100', ip] if sys.platform == 'win32' else ['ping', '-c', '1', '-W', '2', ip]
    ...
    def detect_os(ip: str) -> Dict:
        ...
        try:
            ...
            result['details'].append("Fast response: Likely Linux/Unix" if response_time < 0.1 else "Slow response: Possibly Windows")
        except:
            pass
...
class SSLAnalyzer:
    ...
    def analyze(ip: str, port: int = 443, timeout: float = 5.0) -> Dict:
        result = {'supports_ssl': False, 'versions': [], 'cipher_suites': [], 'certificate': {}, 'vulnerabilities': [], 'grade': 'N/A'}
            ...
            with socket.create_connection((ip, port), timeout=timeout) as sock:
                with context.wrap_socket(sock, server_hostname=ip) as ssock:
                    ...
                    if version in ['SSLv2', 'SSLv3']:
                        result['vulnerabilities'].append(f'Deprecated {version} protocol')
                    
                    result['grade'] = 'A+' if 'TLSv1.3' in str(result['versions']) else 'A' if 'TLSv1.2' in str(result['versions']) else 'C' if 'TLSv1.0' in str(result['versions']) else 'F'
...
class HTTPEnumerator:
    ...
    def get_headers(ip: str, port: int = 80, use_ssl: bool = False) -> Dict:
        result = {'server': 'Unknown', 'content_type': None, 'security_headers': {}, 'methods': [], 'status_code': None}
    ...
    def enumerate_paths(ip: str, port: int = 80, use_ssl: bool = False) -> List[Dict]:
        ...
        for path in HTTP_PATHS:
            ...
            try:
                ...
                req.add_header('User-Agent', 'Port Scanner')
                with urlopen(req, timeout=3) as response:
                    found.append({'path': path, 'status': response.status, 'size': response.headers.get('Content-Length', 'Unknown')})
            ...
            for rtype in ['A', 'AAAA', 'MX', 'NS', 'TXT', 'CNAME']:
                try:
                    ...
                    for rdata in answers:
                        if rtype == 'MX':
                            result['MX'].append(f"{rdata.exchange} (pref: {rdata.preference})")
                        else:
...
class PingSweep:
    ...
    def sweep(network: str, timeout: float = 1.0) -> List[Dict]:
        ...
        try:
            ...
            def ping_host(ip):
                try:
                    cmd = ['ping', '-n', '1', '-w', str(int(timeout * 1000)), str(ip)] if sys.platform == 'win32' else ['ping', '-c', '1', '-W', str(int(timeout)), str(ip)]
...
# ============== Main Port Scanner ==============
class PortScanner:
    __slots__ = ('target', 'timeout', 'threads', 'scan_type', 'timing', 'ports', 'decoys', 
                  'check_vulns', 'rate_limit', 'resolved_ip', 'results', 'vulnerabilities', 
                  'start_time', 'rate_limiter')
    ...
    def scan_port(self, port: int) -> Dict:
        ...
        result = {
            'port': port, 'protocol': 'tcp', 'state': 'closed',
            'service': COMMON_PORTS.get(port, ('unknown', ''))[0],
            'description': COMMON_PORTS.get(port, ('unknown', ''))[1],
            'banner': '', 'version_info': {"type": "unknown", "version": "unknown"},
            'vulnerabilities': []
        }
    ...
    def _print_result(self, result: Dict):
        ...
        print(f"{GREEN}[+] Port {result['port']:>5}/tcp  "
    ...
    def print_summary(self):
        ...
        if self.vulnerabilities:
            ...
            for v in self.vulnerabilities:
                sev = f"[{RED}{v['severity']}{RESET}]" if v['severity'] == 'HIGH' else f"[{YELLOW}{v['severity']}{RESET}]"
                print(f"  {RED}•{RESET} Port {v['port']} ({v['service']}): {v['vulnerability']} {sev}")
        
        print(f"{CYAN}{'='*60}{RESET}")
    
    def get_results(self) -> Dict:
        return {
            'target': self.target, 'resolved_ip': self.resolved_ip, 'scan_type': self.scan_type,
            'timestamp': datetime.now().isoformat(), 'duration': time.time() - self.start_time,
            'open_ports': self.results, 'vulnerabilities': self.vulnerabilities
        }
...
<body>
    ...
    def export_csv(self, filename: str):
        with open(filename, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Port', 'Protocol', 'State', 'Service', 'Banner', 'Vulnerabilities'])
            
            for port in self.results:
                banner = port.get('banner', '') or 'None'
                vulns = '; '.join([v['vulnerability'] for v in port.get('vulnerabilities', [])]) or 'None'
                writer.writerow([port['port'], 'tcp', port['state'], port['service'], banner[:50], vulns])
...
class NetworkScanner:
    ...
    def scan_network(network: str, ports: List[int] = None, **kwargs) -> List[Dict]:
        try:
            ...
            for ip in net.hosts():
                ip_str = str(ip)
                scanner = PortScanner(ip_str, ports=ports or list(COMMON_PORTS.keys()), **kwargs)
                scanner.resolved_ip = ip_str
                scanner.scan()
                
                if scanner.results:
                    results.append({'ip': ip_str, 'hostname': reverse_dns(ip_str), 'ports': scanner.results})
...
Examples:
  ...
  python SpectraScan.py -t target.com -o report.html -f html
    ...
    kwargs = {
        'timeout': args.timeout, 'threads': args.threads, 'scan_type': args.type,
        'timing': args.timing, 'ports': ports, 'decoys': decoys,
        'check_vulns': args.vuln_check, 'rate_limit': args.rate_limit or 0,
    }
